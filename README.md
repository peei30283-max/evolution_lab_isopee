...
[関連論文][人工知能学会_2026全国大会論文_五十崎正明_final.pdf](https://github.com/user-attachments/files/32148555/_2026._._final.pdf)  
...
- 捕食者と被食者はこの空間の外には出ない
- 捕食者と被食者との距離が一定値以下になったときに、捕食されたとみなす
...
#### ■ 初期戦術リスト
| 戦術名(Predator)    | 戦術タイプ     | 説明文（あなたの最新版）                    |
|------------------|-----------|---------------------------------|
| SEARCH           | 探索・ノイズ系   | 獲物群れの位置を探索し、接触機会を探るフェーズ。        |
| CHASE            | 速度強化系     | 逃走する獲物の未来位置を予測しながら高速で追跡するフェーズ。  |
| FLANK            | 側面圧力系     | 群れの側面へ回り込み、進行方向を曲げる圧力をかけるフェーズ。  |
| ENCIRCLE         | 包囲圧力系     | 群れの外周を旋回し、包囲網を形成するフェーズ。         |
| CLUSTER_ENCIRCLE | 局所包囲圧力系   | 散開した獲物クラスタの重心を中心に局所包囲旋回を行うフェーズ。 |
| INTERCEPT        | 迎撃系       | 獲物の進行方向を予測し、最短経路で迎撃位置へ移動するフェーズ。 |
| ATTACK           | 速度最大化・捕獲系 | 獲物に急接近し、捕獲を試みるフェーズ。             |
| CLOSE_IN         | 密着圧力系     | 群れの外縁に密着し、逃走方向を限定するフェーズ。        |
| PINCER           | 挟撃・包囲圧力系  | 複数方向から同時に圧力をかけ、逃走経路を封じるフェーズ。    |

| 戦術名(Prey)        | 戦術タイプ           | 説明文（あなたの最新版）                         |
|------------------|-----------------|--------------------------------------|
| GROUP_CENTER     | 密度制御系           | 群れ中心へ集まり密度を高めるフェーズ。                  |
| REGROUP_SEPARATE | 密度制御＋軽ノイズ系      | サブグループ単位で安全距離を確保しつつ再集合するフェーズ。        |
| SCATTER          | 速度強化＋大ノイズ系      | 捕食者から散開して密度を下げるフェーズ。                 |
| ESCAPE           | 速度強化＋軽角度偏向系     | 捕食者から最短距離で離脱するフェーズ。                  |
| EVASIVE          | 速度強化＋角度偏向＋軽ノイズ系 | 捕食者の進行方向を読み、回避角度で逃げるフェーズ。            |
| BLOCK_PATH       | 速度低下＋角度内向き系     | 捕食者の進路を横方向に遮断し、子供や群れ中心への接近を妨害するフェーズ。 |
| DODGE_ARC        | 速度強化＋大角度偏向系     | 捕食者の軌道を読み、円弧状に回避して追跡方向を外すフェーズ。       |
| ORBIT            | 密度制御＋角度偏向系      | 群れ中心を周回し、捕食者の接近角度を制御するフェーズ。          |
| ALIGN            | 整列＋軽ノイズ系        | 群れの進行方向を整え、密集を維持するフェーズ。              |
